...
$("h5.card-title").text(); // 파인트

.click({
$(히든인풋).val($("h5.card-title").text());
frm.submit();
	...
	<form name="cupsize" action="<%= ctxPath%>/order/order_detail.ice" method="get">
	  ...
	  <div class="card-deck mb-5">
		...
		<div class="card" id="card_pint">
	      ...
	      <div class="card-body">
	        <h5 class="card-title">${requestScope.product.productname}</h5>
	        <p class="card-text">${requestScope.product.productname}</p>
	        <p class="card-text" style="font-weight: bold; font-size: 15pt;">${requestScope.product.price}</p>
	        <input type="hidden" name="productname"/>
	      	<button type="button" value="주문하기" style="float: right"/>
	      </div>
		</div>
